					...
					const char *buf, size_t count)
{
	...
	now = ktime_get();
	need_commit =
		(((now - priv->commit_end_time) > 20000000 ? true : false) &&
...
/***************************************************************************/
static int
sde_crtc_config_fingerprint_dim_layer(struct drm_crtc_state *crtc_state, int stage)
{
	struct sde_crtc_state *cstate;
...
extern int
sde_plane_check_fingerprint_layer(const struct drm_plane_state *drm_state);
