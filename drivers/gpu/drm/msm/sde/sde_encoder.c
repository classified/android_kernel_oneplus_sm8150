		...
		struct drm_connector_state *conn_state)
{
	...
	if (!ret && sde_conn && drm_atomic_crtc_needs_modeset(crtc_state)) {
		...
		SDE_EVT32(sde_conn_state,
			  ((unsigned long long)sde_conn_state) >> 32, 0x9999);
		...
		struct sde_encoder_kickoff_params *params)
{
	...
	if (sde_enc->cur_master &&
	    (!strcmp(sde_enc->cur_master->connector->name, "DSI-1")))
	...
	SDE_EVT32(_sde_encoder_is_dsc_enabled(drm_enc), sde_enc->cur_master,
		  sde_enc->cur_master->cont_splash_enabled);
	...
	if ((dsi_panel_name == DSI_PANEL_SAMSUNG_S6E3HC2) ||
	    (dsi_panel_name == DSI_PANEL_SAMSUNG_SOFEF03F_M)) {
		if (disp_info->intf_type == DRM_MODE_CONNECTOR_DSI &&
		    !_sde_encoder_is_dsc_enabled(drm_enc)) {
			pr_err("DSC is disabled\n");
			if (sde_enc && sde_enc->phys_encs[0] &&
			    sde_enc->phys_encs[0]->connector) {
				SDE_EVT32(sde_connector_get_topology_name(
						  sde_enc->phys_encs[0]->connector), 0x9999);
			}
