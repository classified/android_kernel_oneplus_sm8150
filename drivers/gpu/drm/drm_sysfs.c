...
static ssize_t dc_mode_event_num_read(struct file *file, char __user *user_buf,
...
static ssize_t dc_mode_report_enable_read(struct file *file,
...
static ssize_t dc_mode_report_enable_write(struct file *file,
...
int drm_sysfs_init(void)
{
	...
	prEntry_tmp = proc_create("dc_mode_event_num", 0664, prEntry_dc,
				  &dc_mode_event_num_fops);
	if (prEntry_tmp == NULL) {
	...
	prEntry_tmp = proc_create("dc_mode_report_enable", 0666, prEntry_dc,
				  &dc_mode_report_enable_fops);
	if (prEntry_tmp == NULL) {
...
static ssize_t acl_show(struct device *dev, struct device_attribute *attr,
			char *buf)
{
...
static ssize_t acl_store(struct device *dev, struct device_attribute *attr,
			 const char *buf, size_t count)
{
	struct drm_connector *connector = to_drm_connector(dev);
	int ret = 0;
...
static ssize_t aod_show(struct device *dev, struct device_attribute *attr,
			char *buf)
{
	struct drm_connector *connector = to_drm_connector(dev);
	int ret = 0;
...
static ssize_t aod_store(struct device *dev, struct device_attribute *attr,
			 const char *buf, size_t count)
{
	struct drm_connector *connector = to_drm_connector(dev);
	int ret = 0;
...
static ssize_t DCI_P3_show(struct device *dev, struct device_attribute *attr,
			   char *buf)
{
	struct drm_connector *connector = to_drm_connector(dev);
	int ret = 0;
...
static ssize_t DCI_P3_store(struct device *dev, struct device_attribute *attr,
			    const char *buf, size_t count)
{
	struct drm_connector *connector = to_drm_connector(dev);
	int ret = 0;
...
static ssize_t
native_display_wide_color_mode_show(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	struct drm_connector *connector = to_drm_connector(dev);
	int ret = 0;
	...
	ret = scnprintf(buf, PAGE_SIZE,
			"native display wide color mode = %d\n"
...
static ssize_t
native_display_loading_effect_mode_store(struct device *dev,
					 struct device_attribute *attr,
					 const char *buf, size_t count)
{
	struct drm_connector *connector = to_drm_connector(dev);
...
static ssize_t native_display_loading_effect_mode_show(
...
static ssize_t
native_display_customer_p3_mode_store(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t count)
{
	struct drm_connector *connector = to_drm_connector(dev);
...
static ssize_t
native_display_customer_p3_mode_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	struct drm_connector *connector = to_drm_connector(dev);
	int ret = 0;
...
static ssize_t
native_display_customer_srgb_mode_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	struct drm_connector *connector = to_drm_connector(dev);
...
static ssize_t
native_display_customer_srgb_mode_show(struct device *dev,
				       struct device_attribute *attr, char *buf)
{
	struct drm_connector *connector = to_drm_connector(dev);
	int ret = 0;
...
static ssize_t
native_display_wide_color_mode_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
	struct drm_connector *connector = to_drm_connector(dev);
...
static ssize_t
native_display_srgb_color_mode_show(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	struct drm_connector *connector = to_drm_connector(dev);
	int ret = 0;
	...
	ret = scnprintf(buf, PAGE_SIZE,
			"native display srgb color mode = %d\n"
...
static ssize_t
native_display_srgb_color_mode_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
	struct drm_connector *connector = to_drm_connector(dev);
